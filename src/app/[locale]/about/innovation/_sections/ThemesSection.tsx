...
import { themes as themesData } from "@/data/innovation";
import type { Dictionary } from "@/i18n/types";
import type { Locale } from "@/i18n/config";

interface Props {
  dict: Dictionary["innovation"]["themes"];
  data: Dictionary["data"]["innovationThemes"];
  locale: Locale;
}

export default function ThemesSection({ dict, data }: Props) {
  /* Merge translated text with static label/stat from data file */
  const themes = themesData.map((item, i) => ({
    ...item,
    title: data[i]?.title ?? item.title,
    subtitle: data[i]?.subtitle ?? item.subtitle,
    text: data[i]?.text ?? item.text,
    statLabel: data[i]?.statLabel ?? item.statLabel,
    statSource: data[i]?.statSource ?? item.statSource,
  }));
      ...
      <div className="max-w-7xl mx-auto px-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-16 items-end mb-20">
          <AnimatedSection className="lg:col-span-5">
            ...
            >
              {dict.eyebrow}<br />
              <span className="italic font-normal">{dict.heading1}</span>
            </h2>
          </AnimatedSection>
