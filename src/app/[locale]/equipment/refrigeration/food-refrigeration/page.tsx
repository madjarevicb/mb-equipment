...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    title: dict.metadata.foodRefrigeration.title,
    description: dict.metadata.foodRefrigeration.description,
    ...
    alternates: {
      ...
      languages: { en: "/en/equipment/refrigeration/food-refrigeration", sr: "/sr/equipment/refrigeration/food-refrigeration" },
    },
    openGraph: {
      title: dict.metadata.foodRefrigeration.title,
      description: dict.metadata.foodRefrigeration.description,
    ...
    twitter: { card: "summary_large_image", title: dict.metadata.foodRefrigeration.title, description: dict.metadata.foodRefrigeration.description },
  };
}
...
const pageBrands = refrigerationBrands.filter((b) => ["Dalmec", "Desmon", "Tefcold", "Tehnodom"].includes(b.name));

export default async function FoodRefrigerationPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const dict = await getDictionary(locale as Locale);
  ...
  const jsonLd = {
    ...
    "@graph": [
      {
        "@type": "WebPage",
        name: dict.metadata.foodRefrigeration.title,
        description: dict.metadata.foodRefrigeration.description,
        url: `${COMPANY.url}/${locale}/equipment/refrigeration/food-refrigeration`,
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 4, name: dict.nav.foodRefrigeration, item: `${COMPANY.url}/${locale}/equipment/refrigeration/food-refrigeration` },
        ],
      },
    ],
  };
  ...
  return (
    <>
      ...
      <section className="relative min-h-[50vh] lg:min-h-[60vh] flex items-center overflow-hidden" aria-labelledby="hero-heading">
        <Image src="/images/whatwedo/beverage.jpg" alt="Commercial food refrigeration equipment" fill priority sizes="100vw" className="object-cover" placeholder="blur" blurDataURL={HERO_BLUR} />
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-24 lg:py-32">
          <div className="max-w-2xl">
            ...
            <h1 id="hero-heading" className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.05] mb-6">
              Food<br />
              <span className="italic font-normal">Refrigeration</span>
            </h1>
            <div className="w-16 h-px bg-gold/60 mb-6" />
            <p className="text-white/70 text-lg leading-relaxed max-w-lg font-light">Reach-in refrigerators, blast chillers, undercounter units, and prep tables — precise temperature control for ingredient storage.</p>
          </div>
        </div>
      </section>
          ...
          <AnimatedSection>
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
              <div className="lg:col-span-7">
                <p className="text-text-secondary text-xs font-medium uppercase tracking-[0.3em] mb-6">{dict.nav.foodRefrigeration}</p>
                <h2 id="content-heading" className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-text-primary leading-[1.1]">
                  Store Right,<br />
                  <span className="italic font-normal">Serve Fresh</span>
                </h2>
              </div>
              <div className="lg:col-span-5 flex flex-col justify-end">
                <p className="text-text-secondary leading-relaxed mb-6">Back-of-house refrigeration is the backbone of food safety. We supply Desmon and Infrico reach-in cabinets, blast chillers, undercounter units, and refrigerated prep tables — each sized for your kitchen workflow and storage requirements.</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-end">
              <div className="lg:col-span-7">
                ...
                <h2 id="cta-heading" className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold text-text-primary leading-[1.1] mb-6">
                  Keep It<br /><span className="italic font-normal">Fresh.</span>
                </h2>
                <p className="text-text-secondary leading-relaxed mb-12 max-w-lg">From a single undercounter unit to a complete back-of-house cold chain — we specify and install refrigeration matched to your operation.</p>
