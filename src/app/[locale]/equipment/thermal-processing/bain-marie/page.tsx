...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: dict.metadata.bainMarie.description,
    ...
    openGraph: {
      title: dict.metadata.bainMarie.title,
      description: dict.metadata.bainMarie.description,
      url: `${COMPANY.url}/${locale}/equipment/thermal-processing/bain-marie`,
      siteName: COMPANY.name,
      locale: locale === "sr" ? "sr_RS" : "en_US",
      type: "website",
    },
    twitter: { card: "summary_large_image", title: dict.metadata.bainMarie.title, description: dict.metadata.bainMarie.description },
  };
}
...
export default async function BainMariePage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const dict = await getDictionary(locale as Locale);
  ...
  const jsonLd = {
    ...
    "@graph": [
      {
        "@type": "WebPage",
        name: dict.metadata.bainMarie.title,
        description: dict.metadata.bainMarie.description,
        url: `${COMPANY.url}/${locale}/equipment/thermal-processing/bain-marie`,
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 4, name: dict.nav.bainMarie, item: `${COMPANY.url}/${locale}/equipment/thermal-processing/bain-marie` },
        ],
      },
    ],
  };
    ...
    <>
      ...
      <Breadcrumb
        ...
        items={[
          ...
          { label: dict.nav.bainMarie },
        ]}
      />

      {/* Hero */}
      <section className="relative min-h-[50vh] lg:min-h-[60vh] flex items-center overflow-hidden" aria-labelledby="hero-heading">
        <Image src="/images/whatwedo/steam.jpg" alt="Commercial bain marie and food holding equipment" fill priority sizes="100vw" className="object-cover" placeholder="blur" blurDataURL={HERO_BLUR} />
        ...
        <div className="relative max-w-7xl mx-auto px-6 py-24 lg:py-32">
          <div className="max-w-2xl">
            ...
            <h1 id="hero-heading" className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold text-white leading-[1.05] mb-6">
              Bain Marie<br />
              <span className="italic font-normal">& Holding</span>
            </h1>
            <div className="w-16 h-px bg-gold/60 mb-6" />
            <p className="text-white/70 text-lg leading-relaxed max-w-lg font-light">Temperature-controlled holding equipment for buffet service, banqueting, and à la carte plating stations — food stays at serving temperature, safely.</p>
          </div>
        </div>
      </section>
          ...
          <AnimatedSection>
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
              <div className="lg:col-span-7">
                <p className="text-text-secondary text-xs font-medium uppercase tracking-[0.3em] mb-6">{dict.nav.bainMarie}</p>
                <h2 id="content-heading" className="font-display text-3xl sm:text-4xl lg:text-5xl font-bold text-text-primary leading-[1.1]">
                  Hold Temperature,<br />
                  <span className="italic font-normal">Hold Quality</span>
                </h2>
              </div>
              <div className="lg:col-span-5 flex flex-col justify-end">
                <p className="text-text-secondary leading-relaxed mb-6">Bain maries and holding stations maintain food at precise serving temperatures — critical for buffet service, banquet operations, and high-volume plating. We specify wet and dry heat options sized to your service format.</p>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-end">
              <div className="lg:col-span-7">
                ...
                <h2 id="cta-heading" className="font-display text-4xl sm:text-5xl lg:text-6xl font-bold text-text-primary leading-[1.1] mb-6">
                  Serve at the<br /><span className="italic font-normal">Right Temperature.</span>
                </h2>
                <p className="text-text-secondary leading-relaxed mb-12 max-w-lg">From countertop bain maries to full buffet lines — we specify holding equipment matched to your service format and volume.</p>
