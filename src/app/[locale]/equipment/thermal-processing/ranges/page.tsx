...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: dict.metadata.ranges.description,
    ...
    openGraph: {
      title: dict.metadata.ranges.title,
      description: dict.metadata.ranges.description,
      url: `${COMPANY.url}/${locale}/equipment/thermal-processing/ranges`,
      siteName: COMPANY.name,
      locale: locale === "sr" ? "sr_RS" : "en_US",
      type: "website",
    },
    twitter: { card: "summary_large_image", title: dict.metadata.ranges.title, description: dict.metadata.ranges.description },
  };
}
...
export default async function RangesPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const dict = await getDictionary(locale as Locale);
  ...
  const jsonLd = {
    ...
    "@graph": [
      {
        "@type": "WebPage",
        name: dict.metadata.ranges.title,
        description: dict.metadata.ranges.description,
        url: `${COMPANY.url}/${locale}/equipment/thermal-processing/ranges`,
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 4, name: dict.nav.ranges, item: `${COMPANY.url}/${locale}/equipment/thermal-processing/ranges` },
        ],
      },
    ],
  };
          ...
          <AnimatedSection>
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20">
              <div className="lg:col-span-7">
                <p className="text-text-secondary text-xs font-medium uppercase tracking-[0.3em] mb-6">{dict.nav.ranges}</p>
