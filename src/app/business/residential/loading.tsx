    ...
    <div role="status" aria-label="Loading Residential page" aria-busy="true">
      ...
      >
        <div className="text-center space-y-4">
          <div className="h-4 w-48 bg-white/10 mx-auto" />
          <div className="h-14 w-96 max-w-full bg-white/10 mx-auto" />
          <div className="h-5 w-80 max-w-full bg-white/5 mx-auto" />
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto px-6 space-y-12">
          <div className="flex gap-20">
            {Array.from({ length: 5 }).map((_, i) => (
              <div key={i} className="w-[180px] h-[80px] bg-white/10 flex-shrink-0" />
            ))}
          </div>
          <div className="flex gap-20">
            {Array.from({ length: 5 }).map((_, i) => (
      ...
      <section className="py-16 lg:py-20 bg-navy animate-pulse" aria-hidden="true">
        <div className="max-w-7xl mx-auto px-6 flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex gap-5">
            {[1, 2].map((n) => (
              <div key={n} className="h-24 w-64 bg-white/5" />
            ))}
          </div>
        </div>
      </section>
      ...
      >
        <div className="max-w-4xl mx-auto px-6 flex flex-col items-center gap-4">
          ...
          <div className="h-16 max-w-lg w-full bg-gray-100" />
          <div className="flex gap-4 mt-4">
            <div className="h-12 w-48 bg-gray-200" />
            <div className="h-12 w-36 bg-gray-100" />
          </div>
        </div>
      </section>
