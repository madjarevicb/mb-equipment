...
export default function Header({ locale, nav, common }: HeaderProps) {
  const navItems = getNavItems(locale, nav);
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50">
      ...
      {/* Main nav */}
      <nav aria-label={common.mainNavigation} className="bg-navy/95 backdrop-blur-md border-b border-white/5">
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between h-16 lg:h-20">
          ...
          <div className="hidden lg:flex items-center gap-8">
            ...
            <LanguageSwitcher currentLocale={locale} common={common} />
            <a href={`tel:${COMPANY.phone}`} className="text-white/40 hover:text-white transition-colors duration-300" aria-label={common.callUs}>
          ...
          <div className="flex items-center gap-3 lg:hidden">
            <LanguageSwitcher currentLocale={locale} common={common} />
            <a href={`tel:${COMPANY.phone}`} className="text-white/60 p-2" aria-label={common.callUs}>
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
            </a>
            <MobileMenu navItems={navItems} locale={locale} nav={nav} common={common} />
          </div>
        </div>
      </nav>
    </header>
