...
export default function SectionHeading({ overline, heading, subtext, align = "center", theme = "light" }: SectionHeadingProps) {
  ...
  const headingColor = theme === "dark" ? "text-white" : "text-[#212529]";
  const subtextColor = theme === "dark" ? "text-gray-300" : "text-[#595F66]";

  return (
    <div className={`flex flex-col gap-4 ${alignClass}`}>
      {overline && <span className="text-sm font-semibold uppercase tracking-[0.2em] text-[#C9A84C]">{overline}</span>}
